...
const BillPay = () => {
  ...
  useEffect(() => {
    if (singleBillPay?.data) {
      reset({
        supplierId: singleBillPay?.data?.supplierId || "",
        name: singleBillPay?.data?.name || "",
        mobile_number: singleBillPay?.data?.mobile_number || "",
        address: singleBillPay?.data?.address || "",
        email: singleBillPay?.data?.email || "",
        shop_name: singleBillPay?.data?.shop_name || "",
        against_bill: singleBillPay?.data?.against_bill || "",
        category: singleBillPay?.data?.category || "",
        amount: singleBillPay?.data?.amount || "",
        payment_against_bill: singleBillPay?.data?.payment_against_bill || "",
        paid_on: singleBillPay?.data?.paid_on || "",
        individual_markup: singleBillPay?.data?.individual_markup || "",
        payment_method: singleBillPay?.data?.payment_method || "",
        selected_bank: singleBillPay?.data?.selected_bank || "",
        bank_account_no: singleBillPay?.data?.bank_account_no || "",
        check_no: singleBillPay?.data?.check_no || "",
        card_number: singleBillPay?.data?.card_number || "",
        card_holder_name: singleBillPay?.data?.card_holder_name || "",
        card_transaction_no: singleBillPay?.data?.card_transaction_no || "",
        card_type: singleBillPay?.data?.card_type || "",
        month_first: singleBillPay?.data?.month_first || "",
        year: singleBillPay?.data?.year || "",
        month_second: singleBillPay?.data?.month_second || "",
        security_code: singleBillPay?.data?.security_code || "",
        transaction_no: singleBillPay?.data?.transaction_no || "",
        transactionId: singleBillPay?.data?.transactionId || "",
        expense_note: singleBillPay?.data?.expense_note || "",
      });
    }
  }, [reset, singleBillPay?.data]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await updateBillPay(values).unwrap();
      if (response.success) {
                                ...
                                {loading ? (
                                  ...
                                  <>
                                    {url ? (
                                      ...
                                      <span>
                                        {" "}
                                        {singleBillPay?.data?.image
                                          ? singleBillPay?.data?.image.slice(
                                              0,
                                              20
                                            )
                                          : "Upload File / Image"}
