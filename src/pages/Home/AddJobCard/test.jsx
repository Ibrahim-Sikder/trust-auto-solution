...
const AddJobCard = () => {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.success) {
      ...
      if (clickControl === "quotation") {
        navigate(`/dashboard/qutation?id=${res?.data?._id}`);
      }
      if (clickControl === "invoice") {
        navigate(`/dashboard/invoice?id=${res?.data?._id}`);
      }
  ...
  //     try {
  //       const apiEndpoints = [
  //         `${import.meta.env.VITE_API_URL}/api/v1/customer/${userId}`,
  //         `${import.meta.env.VITE_API_URL}/api/v1/company/${userId}`,
  //         `${import.meta.env.VITE_API_URL}/api/v1/showRoom/${userId}`,
  //       ];
